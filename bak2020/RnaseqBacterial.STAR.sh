...
perl -MCPAN -Mlocal::lib -e 'CPAN::install(GD::Graph::bars)'
...
# echo STAR $OPTS_P1 $OPTS_P2 --runThreadN 24 --outBAMsortingThreadN 6 $ZCAT --genomeDir $DBDIR  --sjdbGTFfile ~/hpc/db/hg19/STAR/gencode.v32lift37.annotation.gtf --outFileNamePrefix ./human/$i --readFilesIn $i\_R1_001.fastq.gz $i\_R2_001.fastq.gz >> $i.job
...
#echo trim_galore --paired --phred33 --clip_R1 0 --clip_R2 0 --fastqc --illumina $i\_R1_001.fastq.gz $i\_R2_001.fastq.gz --output_dir ./fastq_trim >> $i.job
...
#echo fastqc $i\_R1_001.fastq.gz $i\_R2_001.fastq.gz  -o  ./fastqc >>$i.job
#rename .fq.gz .fastq.gz *.fq.gz >> $i.job
#qsub  $i.job
done
